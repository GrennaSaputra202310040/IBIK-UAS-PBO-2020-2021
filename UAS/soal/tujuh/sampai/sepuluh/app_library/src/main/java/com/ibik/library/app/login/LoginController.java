...
public class LoginController {
	...
	public LoginController() {
		...
		try {
			...
			conn.connect();
			initialize();
		} catch (SQLException e) { // error handling for if connection is connected will show window dialog, the
	...
	private void initialize() {
		...
		labelIcon.setIcon(new ImageIcon(
				"C:\\CODING\\PBO\\app_library\\src\\main\\java\\com\\ibik\\library\\app\\images\\logo-ibik-web.png"));
		...
		btnLogin.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				CallbackResultSet result = new LoginController().ValidationForm(username, password);
		...
		JLabel labelCopyright = new JLabel("(c) 2021 Grenna Saputra");
		labelCopyright.setBounds(10, 430, 149, 14);
		frameMain.getContentPane().add(labelCopyright);
	}

	public CallbackResultSet ValidationForm(String username, String password) {
		CallbackResultSet m = new CallbackResultSet();
